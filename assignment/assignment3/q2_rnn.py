...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if delta > 0:
            sentence.append(zero_vector * delta)
            labels.append(zero_label * delta)
            mask.append(False * delta)
        elif delta < 0:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed_temp = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embed_temp, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable("U", shape=(self.config.hidden_size, self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer(seed=1))
        b2 = tf.get_variable("b2", shape=(self.config.n_classes), initializer=tf.contrib.layers.xavier_initializer(seed=2))
        ### END YOUR CODE
