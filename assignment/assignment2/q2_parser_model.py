...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings = tf.nn.embedding_lookup(tf.constant(self.pretrained_embeddings), self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        weitht_initializer = xavier_weight_init()
        W = weitht_initializer([self.config.n_features*self.config.embed_size, self.config.hidden_size])
        b1 = tf.Variable(tf.zeros([self.config.hidden_size,]))
        U = weitht_initializer([self.config.hidden_size, self.config.n_classes])
        ...
        with tf.Session() as session:
            ...
            if not debug:
                ...
                with open('q2_test.predicted.pkl', 'w') as f:
                    printickle.dump(dependencies, f, -1)
                print("Done!")
